...
jQuery(function($) {
    ...
    UIkit.$win.on('load', function(e) {
        
        $('.tm-navbar [data-uk-dropdown]').each(function(i, ele) {
            console.log('dropdown found');
            var ele = UIkit.$(ele);
            var data = UIkit.Utils.options($.extend(ele.data('ukDropdown'), { justify: 'body' }));

            if (data !== undefined) {
                var dropdown = $.UIkit.dropdown(ele, data);
